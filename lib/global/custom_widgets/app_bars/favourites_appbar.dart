...
class MyFavouritesAppBar extends StatefulWidget implements PreferredSizeWidget {
  final double _height;
  final String _title;
  final MaterialPageRoute _backScreen;
  const MyFavouritesAppBar(this._title, this._height, this._backScreen);

  @override
  _MyFavouritesAppBarState createState() => _MyFavouritesAppBarState();

  @override
  Size get preferredSize => Size.fromHeight(82);
}

class _MyFavouritesAppBarState extends State<MyFavouritesAppBar> {
  int quantity = 0;
  ...
  Widget build(BuildContext context) {
    ...
    CollectionReference _userSaves = FirebaseFirestore.instance.collection('users').doc(_user.phone)
        .collection("user_collections").doc("myFavourites").collection("myFavourites");
